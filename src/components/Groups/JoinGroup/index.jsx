...
export const JoinGroup = () => {
  ...
  const getAllGroupMembers = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/groups/${_id}/members`);
  ...
  const handleAddMember = async () => {
    try {
      const response = await axios.put(`${import.meta.env.VITE_API_URL}/groups/members/${_id}`, {
        member: {
          userId: profile._id,
