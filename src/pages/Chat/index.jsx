...
const Chat = ({userId}) => {
  ...
  return (
    <div style={{ height: '100%', display: 'flex',  flexDirection: 'row', backgroundColor: 'aquamarine',borderRadius:'10px'  }}>
      ...
      <div style={{ backgroundColor: 'dodgerblue', width: '100%', display: 'flex', flexDirection: 'column' }}>
        <div style={{ backgroundColor: '#c3eefa', height: '90%' }}>
          ...
          {!!selectedUserId && (
            <div style={{ position: 'relative', height: '100%' }}>
              <div style={{ position: 'absolute', overflowY: 'scroll', height: '100%', width: '100%',padding:'20px',borderRadius:'10px' }}>
                {messagesWithoutDupes.map(message => (
                  <div key={message._id} style={{ textAlign: (message.sender === profile._id ? 'right' : 'left') }}>
                    <div className={"" + (message.sender === profile._id ? 'myChat' : 'yourChat')} style={{ textAlign: 'left', borderRadius: '6px', padding: '10px', margin: '5px 60px 5px 60px' }}>
        ...
        {!!selectedUserId && (
          <div style={{ backgroundColor: 'floralwhite', height: '10%', width: '100%' }}>
           ...
           <form style={{ height: '100%',display: 'flex',alignItems: 'center',gap: '0.5vw' }} onSubmit={sendMessage}>
              <input value={newMessageText} onChange={ev => setNewMessageText(ev.target.value)} type="text" style={{ height: '100%', width: '85%',borderRadius:'100px',backgroundColor:'#DCDCDC',marginLeft:'5px',paddingLeft:"20px",borderColor:'ActiveBorder' }} placeholder="Message here " ></input>
