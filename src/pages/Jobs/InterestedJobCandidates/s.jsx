...
const IndividualJobPost = () => {
    ...
    function MyVerticallyCenteredModal(props) {
        ...
        const handleSubmit = () => {
            ...
            const apiUrl = `${import.meta.env.VITE_API_URL}/jobs/apply/${_id}`;
    ...
    const handleStatusUpdate = (status, comment, userId) => {
        ...
        axios.put(`${import.meta.env.VITE_API_URL}/jobs/${_id}/updateJobStatus`, { userId, status, comment })
            .then(response => {
    ...
    const RenderCandidateDetails = () => {
        ...
        return appliedCandidatesDetails.map((candidate, index) => (
            <div key={index}>
                ...
                <div style={{ display: 'flex', gap: '1vw' }}>
                    <p style={{ fontWeight: '500' }}>Resume: </p><a href={`${import.meta.env.VITE_API_URL}/uploads/${candidate.resume}`} target="_blank" rel="noopener noreferrer">{candidate.resume}</a>
                </div>
                <div style={{ display: 'flex', gap: '1vw' }}>
            ...
            if (attachment.endsWith('.pdf')) {
                ...
                return (
                    <div key={index} className="image-link">
                        <button style={{ border: 'none', borderBottom: 'solid 1px' }} onClick={() => handleImageClick(`${import.meta.env.VITE_API_URL}/uploads/${attachment}`)}>
                            {attachment}
                        </button>
                    </div>
                );
